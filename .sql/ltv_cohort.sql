...
-- doan nay dung de lay order, nmv, customer_key
order_data as (
select 
  * 
from(
    select
      ...
      SUM(CASE 
        ...
        END) as nmv,
      COUNT(original_code) as num_items
...
left join activation_data f on md.first_quarter = f.year_quarter and md.activation_channel = f.channel
where substr(md.first_quarter,1,4) >= '2017'
order by fyear, fquarter, subs
)

select
  a.* except(activation_customer, nmv_activation_customer),
